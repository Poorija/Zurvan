# Zurvan + AI - The Modern Security Interface with AI
...
- **OSINT Tab:** A new centralized tab for Open Source Intelligence gathering, featuring tools like `theHarvester`, `Sherlock`, `Spiderfoot`, `Magma OSINT`, `Photon`, `PhoneInfoga`, and `Metagoofil`.
- **New Tools:** Added `WhatWeb` for web technology identification and `TruffleHog` for secret scanning.
- **Enhanced Nmap:** Added more firewall evasion options to the Nmap scanner.
- **New Report Template:** Added a new "Professional Blue" HTML report template.
...
### 3. OSINT Tab
- A centralized tab for Open Source Intelligence gathering, featuring a suite of powerful tools:
    - **theHarvester:** Gather emails, subdomains, hosts, and employee names from public sources.
    - **Sherlock:** Hunt for social media accounts by username across social networks.
    - **Spiderfoot:** Automate OSINT collection to find everything about a target.
    - **Magma OSINT:** A comprehensive OSINT tool for scanning various online resources.
    - **Photon:** An incredibly fast crawler designed for OSINT.
    - **PhoneInfoga:** Scan phone numbers using only free resources.
...
- **Nmap Scanner:** A comprehensive frontend for the Nmap network scanner.
    ...
    - **Enhanced Firewall Evasion:** More options to bypass firewalls.
...
- **TruffleHog:** A secret scanner to find leaked credentials in git repositories, GitHub, and local filesystems.
