...
void *
ip6_opt_open(int argc, char *argv[], struct rule *rule)
{
...
int
ip6_opt_apply(void *d, struct pktq *pktq, struct rule **next_rule)
{
...
#if 0
			printf("len: %d, nxt %d, data %02x %02x %02x %02x %02x %02x %02x %02x\n",
					opt->u.raw.len, opt->u.raw.proto,
					opt->u.raw.data8[0], opt->u.raw.data8[1],
